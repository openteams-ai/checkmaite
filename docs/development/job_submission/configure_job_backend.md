# `configure_job_backend(...)` and analytics-store handoff
...
So `configure_job_backend(...)` requires explicit `analytics_store=...` configuration.
...
1. `checkmaite.jobs._api.configure_job_backend(...)` accepts `analytics_store`.
2. `RayJobBackend` validates/stores that config once.
3. `checkmaite.jobs._api.submit_capability(...)` delegates to the active job backend for each job submission.
4. `RayJobBackend.submit_capability(...)` injects the stored config into task kwargs (`_analytics_store`).
...
For broader Ray runtime/reconfiguration behavior (`runtime_env`, `force_reinit`), see [Ray job backend](ray_job_backend.md). For store semantics and URI resolution details, see [Distributed analytics store](analytics_store.md).
