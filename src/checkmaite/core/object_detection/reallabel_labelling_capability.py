...
from checkmaite.core._types import DataFrame, Image
from checkmaite.core._utils import deprecated, requires_optional_dependency
from checkmaite.core.cached_tasks import predict
...
class ReallabelLabellingImageOutput(CapabilityOutputsBase):
...
class ReallabelLabellingOutputs(CapabilityOutputsBase):
    """Results class
    ...
    results: DataFrame
    example_image: ReallabelLabellingImageOutput
...
class ReallabelLabellingConfig(_NativeRealLabelConfig, CapabilityConfigBase):
    """Config class"""

    @model_validator(mode="after")
    def _ensure_aggregated_confidence(self) -> "ReallabelLabellingConfig":
...
class ReallabelLabellingRun(CapabilityRunBase[ReallabelLabellingConfig, ReallabelLabellingOutputs]):
    """Run class.
...
class ReallabelLabelling(
    ...
    Labelling uses an ensemble of models and their inference results to provide insight into the potential
    ...
    This capability also uses MAITE-wrapped models and datasets, and MAITE itself,
    to produce the model inference results needed if they are not present in the
    cache before running RealLabel itself.
    """

    _RUN_TYPE = ReallabelLabellingRun

    @classmethod
    def _create_config(cls) -> ReallabelLabellingConfig:
        return ReallabelLabellingConfig()
    ...
    def _run(
        ...
        """Run Labelling capability.

        Returns
        -------
        ReallabelLabellingOutputs
            The results of the Labelling analysis.

        Raises
        ------
        RuntimeError
            If the Labelling result pyspark DataFrame is empty.
        ValueError
        ...
        maite_inference_result = self._compute_maite_inference_result(
            models=models, dataset=dataset, use_prediction_and_evaluation_cache=use_prediction_and_evaluation_cache
        )
        ...
        # PIL stores as HWC, but MAITE and checkmaite requires CHW
        if isinstance(most_populous_image_array, torch.Tensor):
            ...
            raise RuntimeError(
                f"Labelling Capability Error: image array type not understood ({type(most_populous_image_array)})"
            )
    ...
    def _compute_maite_inference_result(
        self, models: list[od.Model], dataset: od.Dataset, use_prediction_and_evaluation_cache: bool
    ) -> dict[str, od.Dataset]:
        ...
        for model in models:
            ...
            copied_dataset = _LabellingDatasetWrapper(dataset)
            ...
            # supported through checkmaite currently. Additionally, metadata specific to the ground
