"""DataEval Linting Common Test Stage"""
...
class DatasetLintingTestStageBase(TestStage[dict[str, Any]], SingleDatasetPlugin[TDataset]):
    """
    Dataset Linting TestStage Base implementation.
    ...
    def cache_id(self) -> str:
        ...
        return f"linting_{self._task}_{self.dataset_id}.dat"
            ...
            for j, dl in enumerate(nd):
                ...
                if not filepath.exists():
                    for di, ax in zip(dl, axes):
                        ...
                        image, label, box = self._get_image_label_box(ii, ti)
                        title = f"{label}"
                        ax.clear()
                        ax.axis("off")
                        ax.set_title(title)
