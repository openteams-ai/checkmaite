...
# Local imports
from jatic_ri.image_classification.test_stages import NRTKTestStage, NRTKTestStageConfig
...
@pytest.fixture
def test_config():
    return NRTKTestStageConfig(name=ARGS["name"], perturber_factory=ARGS["perturber_factory"])


@pytest.mark.parametrize("use_stage_cache", [True, False])
def test_nrtk_test_stage(use_stage_cache, dummy_model_od, dummy_dataset_od, dummy_metric_od, test_config) -> None:
    """Test NRTKTestStage implementation"""

    test = NRTKTestStage()
    run = test.run(
        use_stage_cache=use_stage_cache,
        models=[dummy_model_od],
        metrics=[dummy_metric_od],
        datasets=[dummy_dataset_od],
        config=test_config,
    )
    ...
    assert example_args["layout_arguments"]["x_data_col"] == test_config.perturber_factory.theta_key
