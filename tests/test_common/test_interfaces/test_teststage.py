...
class ParamTestStage(TestStage):
    """A TestStage whose supported cardinalities are configurable."""
...
def test_teststage_collect_with_run(
    card: Number, fake_od_model_default, fake_od_dataset_default, fake_od_metric_default
) -> None:
    """Parameterized over all Number values; verifies run executes with valid counts,
    and report consumables are returned."""
    # Use the same cardinality for all three axes to keep this test simple.
    stage = ParamTestStage(supports_models=card, supports_datasets=card, supports_metrics=card)
